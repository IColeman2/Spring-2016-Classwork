...
DELETE FROM ProductsInInvoices WHERE Id IN (SELECT i.invoiceCustomer FROM Invoices i WHERE (SELECT c.personId FROM Customers c WHERE (SELECT p.personId FROM Persons p WHERE p.firstName = 'Matt' AND p.lastName = 'RadarTechnician') = c.personId) = i.invoiceCustomer);
DELETE FROM ProductsInInvoices WHERE personId IN (SELECT i.invoiceSalesPerson FROM Invoices i WHERE (SELECT p.personId FROM Persons p WHERE p.firstName = 'Matt' AND p.lastName = 'RadarTechnician') = i.invoiceSalesPerson);
DELETE FROM Invoices WHERE invoiceCustomer IN (SELECT c.customerId FROM Customer c WHERE (SELECT p.personId FROM Persons p WHERE p.firstName = 'Matt' AND p.lastName = 'RadarTechnician') = c.customerId);
...
SELECT pi.productConnectionID AS transactionCode,
	   p.productCode AS productCode,
	   i.invoiceCode AS invoiceCode
       FROM ProductsInInvoices pi
...
SELECT i.invoiceCode AS invoiceCode,
...
SELECT pi.productId FROM ProductsInInvoices pi WHERE (SELECT p.productType FROM Products p WHERE pi.productId = p.productId) = 'equipment' GROUP BY pi.invoiceId, pi.productId HAVING COUNT(*) > 1;
