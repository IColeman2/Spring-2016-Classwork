...
public class InvoiceSystemDriver {
	...
	public static void main(String[] args){
			...
			for (Entry<String, Invoice> invoice: invoiceList.entrySet()) {
				...
				for (Entry<String, Product> Product: temp.getInvoiceProducts().entrySet()) {
					...
					if(temp.getInvoiceCustomer().getCustomerType().equals("Corporate")){
						taxes += currentInvoiceProduct.getTotalCost(temp.getProductUnits().get(currentInvoiceProduct.getProductCode()))*currentInvoiceProduct.getTax() - currentInvoiceProduct.getTotalCost(temp.getProductUnits().get(currentInvoiceProduct.getProductCode()));
						//For testing
						System.out.println("TAXES EQUAL" + taxes);
					}
					//For testing
					else{
						System.out.println("No taxes");
						System.out.println(temp.getInvoiceCustomer().getCustomerType());
					}
					fees += currentInvoiceProduct.getFee();
					subtotal += currentInvoiceProduct.getTotalCost(temp.getProductUnits().get(currentInvoiceProduct.getProductCode()));
					if(currentInvoiceProduct.getProductType().equals("Equipment")) {
						Equipment currentEquipment = (Equipment) driver.getProductsList().get(currentInvoiceProduct.getProductCode()) ;
						String itemString = currentEquipment.getProductName() + " (" + temp.getProductUnits().get(currentInvoiceProduct.getProductCode()) + " units @ $" + 
						Math.round(currentEquipment.getUnitCost()*100.0)/100.0 + "/unit)";
						System.out.printf("%-10s%-70s$%10.2f $%10.2f\n", currentEquipment.getProductCode(), itemString, currentInvoiceProduct.getFee(), currentInvoiceProduct.getTotalCost(temp.getProductUnits().get(currentInvoiceProduct.getProductCode())));
					}
						...
						String itemString = currentLicense.getProductName() + " (" + temp.getProductUnits().get(currentInvoiceProduct.getProductCode()) +
								" days @ $" + currentLicense.getAnnualServiceFee() + "/yr)";
						System.out.printf("%-10s%-70s$%10.2f $%10.2f\n", currentLicense.getProductCode(), itemString,currentInvoiceProduct.getFee(), currentInvoiceProduct.getTotalCost(temp.getProductUnits().get(currentInvoiceProduct.getProductCode())));
						...
						String itemString = currentConsultation.getProductName() + " (" + temp.getProductUnits().get(currentInvoiceProduct.getProductCode()) + " hrs @ " + currentConsultation.getPerHourCost() + " per hour)";
						System.out.printf("%-10s%-70s$%10.2f $%10.2f\n", currentConsultation.getProductCode(), itemString, currentInvoiceProduct.getFee(), currentInvoiceProduct.getTotalCost(temp.getProductUnits().get(currentInvoiceProduct.getProductCode())));
					}
				}
				System.out.println("                                                                                ========================");
	...
	private static ArrayList<String> getEmail(int personId){
		...
		String query = "Select emailAddress FROM Emails WHERE personId = ?";
				try {
					PreparedStatement ps = conn.prepareStatement(query);
	...
	private static String getState(int stateKey){
		String query = "Select stateName FROM States WHERE stateId = ?";
		String state = "";
				try {
					PreparedStatement ps = conn.prepareStatement(query);
	...
	private static double getUnitCost(int key){
		double unitCost = 0;
		String query = "SELECT unitCost FROM Products WHERE productId = ?";
				try {
					PreparedStatement ps = conn.prepareStatement(query);
					ps.setInt(1, key);
					ResultSet rs = ps.executeQuery();
	...
	private static double getPerHourCost(int key){
		...
		String query = "SELECT perHourCost FROM Products WHERE productId = ?";
				try {
					PreparedStatement ps = conn.prepareStatement(query);
					ps.setInt(1, key);
					ResultSet rs = ps.executeQuery();
	...
	private static Person getSalesPerson(int key){
		...
		String query = "SELECT invoiceId FROM ProductsInInvoices WHERE productId = ?";
				try {
					PreparedStatement ps = conn.prepareStatement(query);
					ps.setInt(1, key);
					ResultSet rs = ps.executeQuery();
	...
	private static int getInvoiceSalesPerson(int invoiceId){
		int personId = 0;
		String query = "SELECT invoiceSalesperson FROM Invoices WHERE invoiceId = ?";
				try {
					PreparedStatement ps = conn.prepareStatement(query);
	...
	private static double getAnnualFee(int key){
		double annualFee = 0;
		String query = "SELECT annualServiceFee FROM Products WHERE productId = ?";
				try {
					PreparedStatement ps = conn.prepareStatement(query);
					ps.setInt(1, key);
					ResultSet rs = ps.executeQuery();
	...
	private static double getFlatFee(int key){
		double flatFee = 0;
		String query = "SELECT flatServiceFee FROM Products WHERE productId = ?";
				try {
					PreparedStatement ps = conn.prepareStatement(query);
					ps.setInt(1, key);
					ResultSet rs = ps.executeQuery();
		...
		String query = "SELECT * FROM Customers WHERE customerId = ?";
					try {
						...
						while(rs.next()){
							...
							Person contact = getPerson(personKey);
							Address companyAddress = getAddress(addressKey);
							if(type.equals("government")||type.equals("G")){
								c = new Government(code, name, contact, companyAddress);
							}
							else if(type.equals("corporate")||type.equals("C")||type.equals("company")){
								c = new Corporate(code, name, contact, companyAddress);
							}
							else {
								System.out.print("Customer type not recognized.");
							}
	...
	private HashMap<String, Product> getInvoiceProducts(int invoiceId){
		...
		String query = "SELECT productId FROM ProductsInInvoices WHERE invoiceId = ?";
			try {
				PreparedStatement ps = conn.prepareStatement(query);
				ps.setInt(1, invoiceId);
				ResultSet rs = ps.executeQuery();
	...
	private static HashMap<String, Integer> getProductUnits(int invoiceId){
		...
		String query = "SELECT productId, numberOfUnits, beginningDate, endDate, consultationHours FROM ProductsInInvoices WHERE invoiceId = ?";
			try {
				PreparedStatement ps = conn.prepareStatement(query);
				ps.setInt(1, invoiceId);
				ResultSet rs = ps.executeQuery();
				while(rs.next()){
					...
					if(p.getProductType().equals("License")){
						Date beg = rs.getDate("beginningDate");
						Date end = rs.getDate("endDate");
						int units = (int) TimeUnit.MILLISECONDS.toDays(end.getTime() - beg.getTime());
						invoiceProductUnits.put(p.getProductCode(), units);
					}
				}
				rs.close();
